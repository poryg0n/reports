...
\begin{align}
\begin{split}
 \gamma_{\mbox{\tiny$\rm K$}} \equiv \omega\sqrt{2I_p}/E_0
%\gamma = \sqrt{\frac{I_p}{2U_p}}\nonumber
%U_p = \frac{F_0^2}{4\omega^2}, \hspace{20pt} 
\end{split}
 \nonumber 
\end{align}
$\gamma_{\mbox{\tiny$\rm K$}} < 1 \longrightarrow \mbox{ Tunnel ionization (horizontal channel)}$ \\
$\gamma_{\mbox{\tiny$\rm K$}} > 1 \longrightarrow \mbox{ Multiphoton ionization (vertical channel)}$ \\
$\gamma_{\mbox{\tiny$\rm K$}} \sim 1 \longrightarrow \mbox{ Intermediate regime}$ 
...
and excited states
 \item[$\bullet$] Reduces the intricacies of the field to 
the field strength only
 \item[$\bullet$] Wrongly assumes that the tunneling wavepacket 
...
\vspace{5pt}
\item This paper attempts at 
\begin{enumerate}
\footnotesize
\item proposing a more suitable parameter 
than $\gamma_{\mbox{\tiny$\rm K$}}$ for the description of the interaction
\item using the above, quantify the efficiency of OBI 
\end{enumerate}
...
\begin{split}
 i\frac{d}{dt}\Psi(t) = H(t)\Psi(t), 
   \hspace{20pt} H(t) = H_0 - \boldsymbol{p}\boldsymbol{A}(t), \hspace{20pt}
\boldsymbol{A}(t) = -A_0 \cos^4\left(\frac{\pi t}{NT}\right)\sin (\omega t)\boldsymbol{\hat{x}}, 
\end{split}
 \nonumber \\
\begin{split}
\boldsymbol{E}(t) = -\frac{d}{dt}\boldsymbol{A}(t), \hspace{20pt}
 t \in \left[ -NT/2, NT/2 \right], \hspace{20pt} T = 2\pi/\omega, \hspace{20pt}
%I = E_0^2/2.
\end{split} \nonumber
\end{align}
\begin{itemize}
...
Schr\"odinger equation, we get
leaves us with 
\begin{align}
 \frac{\partial R}{\partial t} &= -\frac{1}{2}
        \Bigg\lbrace R\nabla^2 S +2\nabla R \nabla S \Bigg\rbrace, 
  \nonumber \\
 \frac{\partial S}{\partial t} &= -\frac{1}{2}\left\lbrace (\nabla S)^2 
          + \Bigg( \frac{\nabla^2 R}{R} \Bigg)\right\rbrace - V \nonumber
\end{align}
...
\begin{itemize}
\item For ionizing trajectories, asymptotic behavior is such as 
$\mathcal{E}(\infty) \geqslant 0$
...
\item For vertical ionization, 
$r_{\rm ex} \longrightarrow r_0    
\Longrightarrow \beta \longrightarrow 0$
\item For horizontal ionization, 
$r_{\rm ex} \longrightarrow r_{\rm ad} 
\Longrightarrow \beta \longrightarrow 1$
...
d^3 \boldsymbol{r}_0, \hspace{20pt} 
\rho(\boldsymbol{r}_0,0) = \left| \psi(\boldsymbol{r},0) \right|^2. \nonumber
\end{align}
\begin{itemize}
\footnotesize
...
\item To compare with the Keldysh parameter $\gamma_{\mbox{\tiny$\rm K$}}$, they define 
...
\begin{itemize}
\footnotesize
\item[$\bullet$] $\gamma_q$ has the same limits a
nd meaning as $\gamma_{\mbox{\tiny$\rm K$}}$
\item[$\bullet$] $\gamma_q \longrightarrow 0$ for TI and
$\gamma_q \longrightarrow \infty$ for MPI
\item[$\bullet$] $\gamma_q \sim 1$ for equal TI and MPI efficiency, 
we would then have $\bar{\beta} = 1/2$
\end{itemize}
\end{itemize}
...
\begin{itemize}
\footnotesize
\item Fig. 3(a) Comparison of $\gamma_q$ and $\gamma_{\mbox{\tiny$\rm K$}}$ 
for different intensities and different potentials
\begin{itemize}
\footnotesize
\item[$\bullet$] The difference between the potentials 
is sharp at low intensities. $\gamma_q$ 
is more sensitive than $\gamma_{\mbox{\tiny$\rm K$}}$
\item[$\bullet$] Accross different potentials, while 
$\gamma_{\mbox{\tiny$\rm K$}}$ remains unaffected, 
$\gamma_q$ reveals the peculiarities of the structure 
\end{itemize}
\item Fig. 3(b) Comparison of $\gamma_q$ for different pulse durations
\begin{itemize}
\footnotesize
...
\item Fig. 4(a) While $\gamma_{\mbox{\tiny$\rm K$}}$ shows linearity,
$\gamma_q$ shows nonuniformity 
\begin{itemize}
\footnotesize
\item[$\bullet$] Correspond to channel closings known to suppress ionization
\item[$\bullet$] $m$-photon channel closure is realized when
\end{itemize}
\begin{equation}
 m\omega \leqslant I_p + U_p,  \hspace{20pt} U_p = E^2_0/(4\omega^2).
\nonumber
\end{equation}
\vspace{10pt}
...
\begin{itemize}
\footnotesize
\item[$\bullet$] Sharp drop near resonnance indicating a preference for TI
\item[$\bullet$] Population transfer to $3p$ instead of Rydberg states 
favors TI 
\item[$\bullet$] to $3p$ maintain the $e^-$ within reach 
of a manageable horizontal ionization.
\vspace{5pt}
...
\framesubtitle{Over the barrier ionization via quantum trajectories}
...
\item OBI happens if 
\begin{equation}
\mathcal{E}(t) > I_p  \hspace{20pt}
\mbox{ or }\hspace{20pt} \mathcal{E}(t_{\rm ex}) \leqslant I_p 
\end{equation}

\item $W_{\rm OBI}$ is the OBI ionization probability 
$W_0$ is the ionization probability 
